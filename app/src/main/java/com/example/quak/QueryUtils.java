...
public final class QueryUtils {
    ...
    public static ArrayList<earthQuakeData> extractEarthquakes(String SAMPLE_JSON_RESPONSE) {
        ...
        try {
            System.out.println(SAMPLE_JSON_RESPONSE);
            JSONObject obj1 = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray arr1 = obj1.getJSONArray("features");
            ...
            for (int i = 0; i < arr1.length(); i++) {
                ...
                String loc = obj3.getString("place");
                String time = obj3.getString("time");
                double mag = obj3.getDouble("mag");
                String m;
                String s1,s2;

                if(loc.contains(" of ")){
                    int index = loc.indexOf(" of ");
                    s1=loc.substring(0,index+3);
                    s2=loc.substring(index+4);
                }else{
                    s1="Near the";
                    s2=loc;
                }
                ...
                @SuppressLint("SimpleDateFormat") SimpleDateFormat simpleDateFormat = new SimpleDateFormat("EEE,dd MMM yyyy - hh:mm aaa");

                String DateToShow = simpleDateFormat.format(date);

                String s3,s4;

                int j=DateToShow.indexOf(" - ");
                s3=DateToShow.substring(0,j);
                s4=DateToShow.substring(j+3);
